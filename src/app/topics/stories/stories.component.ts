...
export class StoriesComponent {
    ...
    filteredStories = computed(() => this.storyCodes.value()?.filter((s) => s.type === this.selectedType()));
    storyParts = computed(() => {
        const stages: { code: string; name: string; url: string }[] = [];
        Object.entries(this.storyObject.value()?.parts ?? {}).forEach(([code, stage]) => {
            if (stage.before)
                stages.push({
                    code: `${code} (Before)`,
                    name: `${stage.storyName}`,
                    url: `/stories/${this.selectedStory()}/scripts/${stage.before}.txt`,
                });
            if (stage.after)
                stages.push({
                    code: `${code} (After)`,
                    name: `${stage.storyName}`,
                    url: `/stories/${this.selectedStory()}/scripts/${stage.after}.txt`,
                });
            if (stage.interlude)
                stages.push({
                    code: `${code}`,
                    name: `${stage.storyName}`,
                    url: `/stories/${this.selectedStory()}/scripts/${stage.interlude}.txt`,
                });
        });
